...
class Image
{
	...
	private function resize()
	{
        if (self::WIDTH != $this->imageSize[0] || self::HEIGHT != $this->imageSize[1]) {
            $this->image = $this->resampled($this->image, $this->imageSize);
            if (!$this->image) {
    ...
    private function resampled()
    {
        ...
        if ($this->imageSize[0] > $this->imageSize[1]) {
            $width = 640;
            $height = (int) (640 * $imageSize[1] / $imageSize[0]);
        } else {
            $width = (int) (480 * $imageSize[0] / $imageSize[1]);
            $height = 480;
        }
